...
`tools/list` at `https://gateway.pipeworx.io/blockchair/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/blockchair_stats \
  -H 'Content-Type: application/json' \
  -d '{"chain":"bitcoin"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/blockchair_stats`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
